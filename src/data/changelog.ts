...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    details: [
      "item_prices and product_prices gain qty + unit columns so weighed goods (chicken by kg, fruit by kg) can store per-kg unit prices separately from the total paid",
      "Receipt importer now stores unit_price (e.g. 13.99 zł/kg) instead of the total for that specific tray weight, so two trays of chicken at 0.634 kg and 0.618 kg produce one consistent price-book entry instead of two unrelated totals",
      "When the receipt only gives a total, the importer derives the unit price as total ÷ qty",
      "Stats: 'Total gastado' = Σ(price × qty) so old totals still display correctly; 'Precio medio' stays as the unit-price average so €/kg comparisons hold",
      "New 'Combinar duplicados' debug button on the list settings modal (visible only when the URL includes ?debug=1) — retroactively applies the addItem dedup rule to existing rows, repointing item_prices/comments/history to the oldest item and deleting the orphans",
      "Legacy rows with qty NULL are treated as qty=1 by stats so nothing pre-existing breaks",
    ],
  },
  {
    version: "v1.02",
    ...
    details: [
      "product_key now ignores the brand: 'Leche Pascual' and 'Leche Mercadona' share the canonical key 'leche', so user stats can sum all milk purchases regardless of brand and the new price-by-brand panel can compare them",
      "product_prices.country added (backfilled from users.country) so the price book and per-card avg-price hint filter to the buyer's country — Polish users in Biedronka stop seeing Spanish Carrefour averages",
      "New cascade trigger: when an item's brand or name is edited (e.g. typo fix 'Pscual' → 'Pascual'), the change propagates to every product_prices row mirrored from that item's purchases",
      "ItemDetail stats panel gets a 'Esta lista / Todas mis listas' toggle (persisted in localStorage). The global view aggregates every purchase the user has made of the canonical product across all their lists",
      "Global view also surfaces a 'Precios por marca' panel sorted cheapest first, filtered by the user's country — useful for spotting that Mercadona milk is cheaper than Pascual",
      "addItem dedup: when a non-checked item with the same canonical key already exists in the list, the new add sums the qty into the existing row instead of creating a duplicate (units must match for numeric merge)",
    ],
  },
  {
    version: "v1.01",
    ...
    details: [
      "parse-receipt edge function: bumped max_tokens to 16384 and now returns receipt_too_long (HTTP 413) when the model hits the output cap, instead of failing later with an opaque JSON parse error",
      "Import modal shows a localized 'split into two photos' hint when the edge function reports receipt_too_long",
      "i18n: the 'Done' / 'Hechos' / 'Gotowe' label is now 'Bought' / 'Comprados' / 'Kupione' — clearer semantics for a shopping list. Duplicate-item dialog copy follows the same rename",
    ],
  },
  {
    version: "v1.00",
