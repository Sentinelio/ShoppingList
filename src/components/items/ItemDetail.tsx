...
export default function ItemDetail({
  ...
  const editVariants: React.ReactNode[] = [
    // v1: Classic Form — Labels + inputs apilados (matches lab exactly)
    ...
    // v2: Stepper Buttons — Botones +/- grandes (matches lab exactly)
    ...
    // v3: All-in-one Row — Todo compacto en pocas filas (matches lab exactly)
    <div key="e2" style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
      ...
      {item.photo && <img src={item.photo} style={{ width: "100%", maxHeight: 160, objectFit: "cover", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)" }} alt="" />}
      {saveBtn}
    </div>,

    // v4: Minimal Fields — Sin labels, solo placeholders (matches lab exactly)
    ...
    // v5: Quick Presets — Botones rapidos para cantidades comunes (matches lab exactly)
    <div key="e4" style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
      ...
      <div style={{ fontSize: 9, fontWeight: 700, color: "#555d74", textTransform: "uppercase", letterSpacing: "0.08em" }}>{t(lang, "qty")}</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
        {(unit === "L" || !unit ? ["1L","2L","3L","500ml","250ml"] : unit === "kg" ? ["100g","250g","500g","1kg","2kg"] : ["1×","2×","3×","6×","Custom"]).map(p => {
